...
import { closeDatabase, createDatabase, type Database } from './db/client.ts';
...
async function main({ logger, db, storage, push }: { logger: Logger; db: Database; storage: Storage; push: Push }) {
  ...
  program
    ...
    .action(async (file: string, { drop }: { drop: boolean }) => {
      try {
        await seed({ logger, db, storage }, file, drop);
  ...
  pushCmd
    ...
    .action(async (domain: string, options: { title: string; body: string; subscription?: string }) => {
      ...
      if (subscriptions.length === 0) {
        console.log(
          subscription ? `No subscription ${subscription} on ${domain}` : `No device subscribed to ${domain}`,
        );
  ...
  return program.parseAsync(process.argv).finally(() => closeDatabase(db));
}

const config = envConfig();

assert(config.databaseUrl, new Error('Missing DATABASE_URL — the CLI does not run against the in-memory database'));
...
await main({ logger, db, push, storage });
